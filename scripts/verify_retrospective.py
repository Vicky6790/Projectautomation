"""Verify Retrospective upload → generate → report through the Client App proxy."""
...
def main() -> int:
    ...
    print("retrospective page", page.status_code, page.headers.get("content-type"))
    ...
    generated = client.post(f"/api/v1/retrospective/requests/{handle}/generate")
    ...
    for key in SECTIONS:
        if key not in result:
            ...
            return 1
    if result.get("planned_only") is not True:
        print("generated plan should be planned_only")
        return 1
    ...
    if report.status_code != 200:
        ...
        return 1
    text = report.text
    if "Planned only: yes" not in text:
        print("report missing planned-only flag")
        return 1
    for heading in (
        "Schedule variance",
        "Milestone delivery",
        ...
        if heading not in text:
            ...
            return 1
    print("Retrospective E2E via client proxy: OK", handle)
    return 0
