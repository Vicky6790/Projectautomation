...
async def upload_retrospective(file: UploadFile = File(...)) -> FileRecord:
    filename = file.filename or "upload"
    content = await file.read()
    validate_upload(filename, content, "retrospective")
...
def generate_retrospective_request(handle: str) -> ProcessingResponse:
    job = run_retrospective_generation(handle)
...
def get_retrospective_request(handle: str) -> ProcessingResponse:
    return storage_mod.store.get_job(handle)


@router.get("/requests/{handle}/report")
def download_retrospective_report(handle: str) -> Response:
    job = storage_mod.store.get_job(handle)
    filename, media_type, content = export_report("retrospective", job)
