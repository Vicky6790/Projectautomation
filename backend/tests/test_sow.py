...
def test_analyze_returns_six_categories(client: TestClient, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda _text: AnalysisReport(
            gray_areas=["Reasonable is undefined"],
            risks=["Schedule may slip"],
        ),
    )
    ...
    assert result["gray_areas"] == ["Reasonable is undefined"]
...
def test_analyze_retry_without_reupload(client: TestClient, monkeypatch) -> None:
    ...
    def flaky(_text: str) -> AnalysisReport:
        ...
        return AnalysisReport(risks=["Retry worked"])
    ...
    assert second.json()["result"]["risks"] == ["Retry worked"]
    assert calls["n"] == 2
